...
bool Intersects(khBody* body1, khBody* body2)
{
...
void CreateContacts(khBody* bodies, khContact_t** contacts)
{
    ...
    {
        for (khBody* body2 = body1->next; body2; body2 = body2->next)
        {
            ...
            {
                khContact_t* contact = GenerateContact(body1, body2);
...
{
    khContact_t* contact = (khContact_t*)malloc(sizeof(khContact_t));
    assert(contact);

    memset(contact, 0, sizeof(khContact_t));
